...
.button span:before {
  ...
  opacity: 0;
  top: 0;
  left: -20px; /* Start position for the animation */
  transition: 0.5s;
}
...
#Score {background-color: red;}
#Torschützen {background-color: green;}
#Assists {background-color: blue;}
#Scorerpoints {background-color: orange;}
...
<div class="header">
  <button class="button"><span href="https://mkreuschner.github.io/berlinersc-football-analysis.github.io/"> Back </span></button>
  <img src="https://www.berlinersc-fussball.de/__we_thumbs__/37837_17_LogoInBlock.png?m=1538584477" alt="Berliner SC" style="float: right;">
...
<button class="accordion">
  <h2 id="Takeaways">Mo's Keypoints</h2>
</button>
<div class="panel">
  <p><ol type="I">
      <li>Generell: Stern Britz ist defensiv sehr anfällig, hat sich aber in den letzten Spielen stabilisiert </li>
      <li>Platz: wenn meine Stats richtig sind, spielen wir bei ihnen auf einen größeren KuRa. Sie sind heimstark. Alle drei Siege haben sie zu Hause geholt </li>
      <li>Formation: wahrscheinlich: 4-2-3-1 (siehe unten)</li>
      <li>Abwehr: vermutlich mit 4er Kette</li>
      <li>Unterschiedsspieler: </li>
      <li>Standards
        <ul>
          <li>DEF: bekommen viele Gegentore auch durch Verteidiger, schwach vermutlich</li>
          <li>OFF: meiste Tore über Offensive Spieler</li>
        </ul></li></ol></p>
</div>

<button class="accordion">
  <h2 id="Reale11">Reale 11</h2>
</button>
<div class="panel">
  <p>
...
<button class="accordion">
  <h2 id="Head2head">Head-to-head</h2>
</button>
<div class="panel">
  <p>
```{R echo = FALSE, results='markup'}
...
<button class="accordion">
  <h2 id="LetzteSpiele">Letzte Spiele</h2>
</button>
<div class="panel">
  <p>
```{R echo = FALSE, results='markup'}
...
last_five_games %>%
  ...
  scroll_box(width = "650px", height = "250px", extra_css = " float: center; border-radius: 10px; padding: 5px; ") %>%
  row_spec(0, bold = T, color = "black", background = "yellow") %>%
  kable_material_dark(full_width = F) %>%
  kable_styling(font_size = 18, position = "center") 
  #column_spec(2, color = spec_color(last_five_games$where),
  #            link = "https://haozhu233.github.io/kableExtra/") #%>%
  #column_spec(6, color = "white",
  #            background = spec_color(last_five_games$goals_c[1:8], end = 0.7),
  #            popover = paste("am:", last_five_games$goals_c[1:8]))
...
<div class="panel">
  <p>
```{R echo = FALSE, results='markup'} 
# Function to determine formation based on the 'squad' column , fig.width = 4, fig.height = 3.23
extract_formation_and_subs <- function(squad) {
...
# formation_counts_team <- matchday_data %>%
#   filter(team == selected_team) %>%                 # Select the specified team
...
# ggplot(formation_counts_team, aes(x = "", y = n, fill = formation)) +
...
#   labs(
#     title = paste("Formationen von", selected_team),
#     fill = "Formation"
#   ) +
#   theme_minimal() +
#   theme(plot.background = element_rect(fill = "#3b3c41",color = "#3b3c41",),
#         plot.title= element_text(size=14,color="white",family = "Tahoma"),
...
# Create the pie chart with percentage labels
ggplot(formation_counts_gegner, aes(x = "", y = n, fill = formation)) +
  geom_bar(stat = "identity", width = 1, color = "white") +
  coord_polar("y") +                                # Convert bar chart to pie chart
  geom_text(aes(label = paste0(round(percentage, 1), "%")),  # Add percentage labels
            position = position_stack(vjust = 0.5)) +
  labs(
    title = paste("Formationen von", selected_team2),
    fill = "Formation"
  ) +
  theme_minimal() +
  theme(plot.background = element_rect(fill = "#3b3c41",color = "#3b3c41",),
...
<button class="accordion">
  <h2 id="Erwartete11">Erwartete Start11</h2>
</button>
<div class="panel">
  <p>
```{R echo = FALSE, results='markup'}
# Define the team to analyze
selected_team2 <- Gegner  # Replace with the actual team name
...
position_requirements2 <- table(formations[[most_used_formation]])

#print(position_requirements)

# Part 3: Build Expected Squad Based on Formation Requirements and Weighted Score
expected_squad2 <- player_data %>%
  filter(Team == selected_team2) %>%                   # Filter by the selected team
  filter(most_position %in% names(position_requirements2)) %>%  # Keep only players with positions in the formation
  group_by(most_position) %>%
  arrange(desc(weighted_score)) %>%
  mutate(rank = row_number()) %>%                      # Rank players by `weighted_score` within each position
  filter(rank <= position_requirements2[most_position]) %>%  # Select top players based on position requirements
  ungroup() %>%
  arrange(match(most_position, formations[[most_used_formation]]))  # Order by the formation's position order

# Add a Rank column to the top_scorers data frame
expected_squad2 <- expected_squad2 %>%
  mutate(Rank = row_number()) %>%
  select(Nummer, Name, weighted_score, most_position, position_percentage,second_position)  # Select desired columns

# Display the formatted table using formattable
formatted_exp11_gegner <- formattable(
  expected_squad2,
  align = c("c", "l", "c", "c", "c"),
  list(
    # Highlight rows based on rank
    position_percentage = formatter("span", style = x ~ style(
      display = "block",
      padding = "5px",
      `border-radius` = "4px",
      background = ifelse(x=="100%", "lightgreen", ifelse( x >= 75, "lightgreen", ifelse(x >= 50 & x < 75, "yellow", ifelse(x < 50, "lightcoral", "transparent"))))
    ))
  )
)

colnames(formatted_exp11_gegner)[colnames(formatted_exp11_gegner) %in% c("Nummer", "Name", "weighted_score", "most_position", "position_percentage","second_position")] <- c("Nr", "Name", "Score", "Pos 1", "%","Pos 2") 

 # Display the formatted table without row indices
formatted_exp11_gegner %>%
  kbl(align = c("c", "l", "c","c", "c", "c")) %>%
  kable_material_dark("hover", full_width = T,bootstrap_options = c("striped", "hover", "responsive")) %>%
  row_spec(0, bold = T, color = "black", background = "yellow") %>%
...
<button class="accordion">
<h2 id="Spielerstatistik">Spielerstatistik</h2>
</button>
<div class="panel">
  <p>
<button class="tablink" onclick="openStat('Score', this, 'red')" id="defaultOpen">Score</button>
<button class="tablink" onclick="openStat('Torschützen', this, 'green')">Torschützen</button>
<button class="tablink" onclick="openStat('Assists', this, 'blue')">Assists</button>
<button class="tablink" onclick="openStat('Scorerpoints', this, 'orange')">Scorerpoints</button>
...
best_players <- player_data %>%
  ...
  arrange(desc(weighted_score)) %>%                   # Sort by Scorerpoints in decreasing order
...
colnames(formatted_top_scorers)[colnames(formatted_top_scorers) %in% c("Rank", "Nummer", "Name", "most_position", "Spielzeit","weighted_score")] <- c("Rank", "Nr", "Name", "Pos 1", "Spielzeit","Score") 
...
formatted_top_scorers <- formattable(
  ...
  list(
    ...
    Rank = formatter("span", style = x ~ style(
      ...
      padding = "5px",
      `border-radius` = "4px",
      background = ifelse(x == 1, "lightgreen", ifelse(x == 2, "lightgreen", ifelse(x == 3, "lightgreen", "transparent")))
    ))
  )
)
