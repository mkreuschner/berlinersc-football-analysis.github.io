...
gesamttabelle <- gesamttabelle %>%
    ...
    summarise(
      Spiele = n(),
      Siege = sum(goals_s > goals_c),
      Unentschieden = sum(goals_s == goals_c),
      Niederlagen = sum(goals_s < goals_c),
      Tore = sum(goals_s),
      Gegentore = sum(goals_c)
    ) %>%
    mutate(
      Differenz = Tore - Gegentore,          # Tordifferenz berechnen
      Punkte = Siege * 3 + Unentschieden    # Punkte berechnen
    ) %>%
    arrange(desc(Punkte), desc(Differenz)) %>%  # Tabelle nach Punkten und Differenz sortieren
    mutate(Rank = row_number()) %>%             # Rank-Spalte hinzufügen
    select(Rank, everything())                  # Rank-Spalte an den Anfang verschieben
...
rasentabelle  <- rasentabelle  %>%
    ...
    mutate(
      Differenz = Tore - Gegentore,          # Tordifferenz berechnen
      D_Punkte = round((Siege * 3 + Unentschieden) / Spiele ,2)   # Punkte berechnen
    ) %>%
    arrange(desc(D_Punkte), desc(Differenz))   %>%  # Tabelle nach Punkten und Differenz sortieren
    mutate(Rank = row_number()) %>%             # Rank-Spalte hinzufügen
    select(Rank, everything())                  # Rank-Spalte an den Anfang verschieben
...
formtabelle <- last_five_games_1 %>%
  ...
  summarise(
    Spiele = n(),
    Ergebnis_5 = Ergebnis[5],  # Fünftletztes Spiel
    Ergebnis_4 = Ergebnis[4],  # Viertletztes Spiel
    Ergebnis_3 = Ergebnis[3],  # Drittletztes Spiel
    Ergebnis_2 = Ergebnis[2],  # Vorletztes Spiel
    Ergebnis_1 = Ergebnis[1],  # Letztes Spiel
    Tore = sum(goals_s),
    Gegentore = sum(goals_c),
    Dif = Tore - Gegentore,
    Durchschnittspunkte = round(mean(Punkte, na.rm = TRUE), 3)  # Durchschnittspunkte berechnen
  ) %>%
  arrange(desc(Durchschnittspunkte), desc(Dif))  # Tabelle nach Durchschnittspunkten und Differenz sortieren
...
head2head_data <- gesamttabelle %>%
  select(team, Rank, Siege, Unentschieden, Niederlagen) %>%
  mutate(
    Ges_Bilanz = paste(Siege, Unentschieden, Niederlagen, sep = "-")  # Bilanz der Gesamttabelle
  ) %>%
  left_join(
    heimtabelle %>%
      mutate(
        H_Bilanz = paste(Siege, Unentschieden, Niederlagen, sep = "-")  # Heim-Bilanz
      ) %>%
      select(team, Rank_H = Rank , H_Bilanz),
    by = "team"
  ) %>%
  select(team, Rank, Ges_Bilanz, Rank_H, H_Bilanz) %>%
  left_join(
    auswärtstabelle %>%
      mutate(
        A_Bilanz = paste(Siege, Unentschieden, Niederlagen, sep = "-")  # Aus-Bilanz
      ) %>%
      select(team, Rank_A = Rank , A_Bilanz),
    by = "team"
  ) %>%
  select(team, Rank, Ges_Bilanz, Rank_H, H_Bilanz, Rank_A, A_Bilanz) %>%
  left_join(
    rasentabelle %>%
      mutate(
        NR_Bilanz = paste(Siege, Unentschieden, Niederlagen, sep = "-")  # Rasen-Bilanz
      ) %>%
      select(team, Rank_NR = Rank , NR_Bilanz),
    by = "team"
  ) %>%
  select(team, Rank, Ges_Bilanz, Rank_H, H_Bilanz, Rank_A, A_Bilanz, Rank_NR, NR_Bilanz) %>%
  left_join(
    kunstrasentabelle %>%
      mutate(
        KR_Bilanz = paste(Siege, Unentschieden, Niederlagen, sep = "-")  # KuRa-Bilanz
      ) %>%
      select(team, Rank_KR = Rank , KR_Bilanz),
    by = "team"
  ) %>%
  select(team, Rank, Ges_Bilanz, Rank_H, H_Bilanz, Rank_A, A_Bilanz, Rank_NR, NR_Bilanz, Rank_KR, KR_Bilanz) %>%
  left_join(
    kleinkunstrasentabelle %>%
      mutate(
        kKR_Bilanz = paste(Siege, Unentschieden, Niederlagen, sep = "-")  # KuRa-Bilanz
      ) %>%
      select(team, Rank_kKR = Rank , kKR_Bilanz),
    by = "team"
  ) %>%
  select(team, Rank, Ges_Bilanz, Rank_H, H_Bilanz, Rank_A, A_Bilanz, Rank_NR, NR_Bilanz, Rank_KR, KR_Bilanz, Rank_kKR, kKR_Bilanz) %>%
  left_join(
    formtabelle %>%
      mutate(
        letzte5 = paste(Ergebnis_5, Ergebnis_4, Ergebnis_3,Ergebnis_2,Ergebnis_1, sep = "-"), # Form
        tore_letzte5 = paste(Tore, Gegentore, sep = ":")
      ) %>%
      select(team, letzte5 , tore_letzte5),
    by = "team"
  ) %>%
  select(team, Rank, Ges_Bilanz, Rank_H, H_Bilanz, Rank_A, A_Bilanz, Rank_NR, NR_Bilanz, Rank_KR, KR_Bilanz, Rank_kKR, kKR_Bilanz, letzte5 , tore_letzte5) %>%
  left_join(
    formation_per_team %>%
      select(team, Formation , Wahrscheinlichkeit),
    by = "team"
  ) %>%
  select(team, Rank, Ges_Bilanz, Rank_H, H_Bilanz, Rank_A, A_Bilanz, Rank_NR, NR_Bilanz, Rank_KR, KR_Bilanz, Rank_kKR, kKR_Bilanz, letzte5 , tore_letzte5, Formation , Wahrscheinlichkeit) %>%
  left_join(
    abwehrkette %>%
      select(team, dreierKette , viererKette),
    by = "team"
  ) %>%
  select(team, Rank, Ges_Bilanz, Rank_H, H_Bilanz, Rank_A, A_Bilanz, Rank_NR, NR_Bilanz, Rank_KR, KR_Bilanz, Rank_kKR, kKR_Bilanz, letzte5 , tore_letzte5, Formation , Wahrscheinlichkeit, dreierKette , viererKette)
