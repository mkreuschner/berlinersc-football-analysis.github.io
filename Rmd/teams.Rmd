...
<link rel="stylesheet" href="/Users/mkreuschnervsp/git/berlinersc-football-analysis.github.io/styles/teams.css">
...
<div class="w3-col s3">
<a href="stats.html" class="w3-button w3-block w3-black">STATS</a>
</div>
<div class="w3-col s3">
...
print("Mo's Keypoints nur anschauen, wenn wir gegen den Gegner am Wochenende spielen")
...
```
  ...
  <tr>
    <td>Generell</td>
    <td>Köpenick ist offensiv gut aufgestellt, aber defensiv nicht gut (28:28). Heimbilanz ist ausgeglichen - aufpassen: letzte zwei Spiele 15:2 Tore gg. Al-Dersim und Concordia. Es dürfte ein enges Spiel werden, mit wenig Torchancen auf dem großen Platz.</td>
  </tr>
  <tr>
    <td>Platz</td>
    <td>zu Hause. Wir spielen auf einem großen Rasenplatz, zumindest angesetzt (Bilanz: 2-3-3)</td>
  </tr>
  <tr>
    <td>Formation</td>
    <td>wahrscheinlich: 4-4-2 (recht wahrscheinlich: 60%)</td>
  </tr>
  <tr>
    <td>Abwehr</td>
    <td>definitiv 4er Kette, haben vor allem 4-4-2 oder 4-5-1 gespielt.</td>
  </tr>
  <tr>
    <td>Spieler</td>
    <td>Ilinseer (ST). Sie schießen auch wie wir auffällig mehr Tore in der zweiten HZ</td>
  </tr>
  <tr>
    <td>def Standards</td>
    <td>sind vermutlich anfällig bei gegnerischen Standards</td>
  </tr>
  <tr>
    <td>off Standards</td>
...
formatted_info %>%
  ...
  row_spec(0, bold = TRUE, color = "white", background = "black") %>% # Kopfzeile bleibt weiß und schwarz hinterlegt
  row_spec(1:nrow(formatted_info), color = "black") %>%               # Schriftfarbe der restlichen Zeilen auf Schwarz
...
last_five_games %>%
  ...
  scroll_box(width = "100%", height = "20rem", extra_css = "float: center; border-radius: 1rem; padding: 1rem; ") %>%
  row_spec(0, bold = TRUE, color = "white", background = "black") %>% # Kopfzeile bleibt weiß und schwarz hinterlegt
  row_spec(1:nrow(last_five_games), color = "black") %>% 
...
ggplot(formation_counts_gegner, aes(x = reorder(formation,-n), y = n, fill = formation)) +
  ...
  scale_fill_manual(
    ...
    breaks = c("4-4-2","4-5-1","3-4-3","4-2-2-2", "3-2-3-2")
    )
...
expected_squad %>%
  ...
  kable_material_dark("hover", full_width = T,bootstrap_options = c("condensed","striped", "hover", "responsive")) %>%
  row_spec(0, bold = TRUE, color = "white", background = "black") %>% 
  row_spec(1:nrow(expected_squad), color = "black") %>%
...
last_three_squads %>%
  kbl(align = c("l")) %>%
  kable_material_dark("hover", full_width = T, bootstrap_options = c("striped", "hover", "responsive")) %>%
  row_spec(0, bold = TRUE, color = "white", background = "black", font_size = 14) %>% 
  row_spec(1:nrow(last_three_squads), color = "black") %>%  
...
ggplot(interval_data, aes(x = interval_label)) +
  ...
  #scale_y_continuous(labels = abs) +            # Display absolute values on y-axis
  scale_fill_manual(values = c("GT" = "#ff9a9a","T" = "#9afffc")) +
  theme_minimal() +
  theme(plot.background = element_rect(fill = "#3b3c41",color = "#3b3c41",)) +
  theme(plot.title= element_text(size=16,color="white",family = "Tahoma")) +
  theme(axis.title.x = element_text(size=12, color="white",angle=0)) +
...
tore_gegentore %>%
   ...
   row_spec(0, bold = TRUE, color = "white", background = "black") %>% 
   row_spec(1:nrow(tore_gegentore), color = "black") %>%
...
<a href="javascript:void(0)" onclick="openMenu(event, 'Score');" id="myLink">
  <div class="w3-col s6 tablink">Score</div>
</a>
<a href="javascript:void(0)" onclick="openMenu(event, 'Torschützen');">
  <div class="w3-col s6 tablink">Torschützen</div>
</a>
<a href="javascript:void(0)" onclick="openMenu(event, 'Assists');">
  <div class="w3-col s6 tablink">Assists</div>
</a>
<a href="javascript:void(0)" onclick="openMenu(event, 'Scorerpoints');">
  <div class="w3-col s6 tablink">Scorerpoints</div>
</a>
</div>
<div id="Score" class="w3-container menu w3-padding-48 w3-card">
```{R echo = FALSE, results='markup'}
...
formatted_top_scorers <- formattable(
  ...
  list(
    ...
    Rank = formatter("span", style = x ~ style(
      ...
      padding = "5px",
      `border-radius` = "4px",
      background = ifelse(x == 1, "#a5ff9a", ifelse(x == 2, "#a5ff9a", ifelse(x == 3, "#a5ff9a", "transparent"))),
...
<div id="Torschützen" class="w3-container menu w3-padding-48 w3-card">
...
<div id="Assists" class="w3-container menu w3-padding-48 w3-card">
```{R echo = FALSE, results='asis'}
...
<div id="Scorerpoints" class="w3-container menu w3-padding-48 w3-card">
```{R echo = FALSE, results='asis'}
